...
int main(int argc, char *argv[])
{
    ...
    unsigned long long int maxG = (1ULL << (N - 1));
    int *cptrs;
    int *rows;
    double *cvals;
    ...
    unsigned long long int chunksize = (1ULL << (N - 1)) - 1ULL;

    chunksize = floor((chunksize) / 512);
...
#pragma omp parallel for schedule(dynamic, chunksize) reduction(+ : p) proc_bind(spread)
    for (unsigned long long int g = 1; g < maxG; g++)
    {
        ...
        memcpy(myX, X, N * sizeof(double));
        unsigned long long int myStart = g;
        unsigned long long int prevGray = (myStart - 1) ^ ((myStart - 1) >> 1);
        double myP = 0;
        int colIdx = 0;
        if (g != 1)
        {
            while (prevGray != 0)
            {
                if (prevGray & 1 == 1)
                {
                    for (unsigned long long int ptr = cptrs[colIdx]; ptr < cptrs[colIdx + 1]; ptr++)
                    {
                        int row = rows[ptr];
                        double val = cvals[ptr];
                        myX[row] = myX[row] + val;
        ...
        long long int myEnd = g + chunksize;
        myEnd = (myEnd < maxG) ? myEnd : maxG;
        for (; g < myEnd; g++)
        {
            long long int grayNow = (g >> 1) ^ g;
            long long int grayPrev = ((g - 1) >> 1) ^ (g - 1);
            int j = __builtin_ctzll(grayNow ^ grayPrev);
            int s = ((grayNow >> (j)) & 1) == 1 ? 1 : -1;
    ...
    tempResult = p * (4 * (N & 1) - 2);
    double endT = omp_get_wtime();
    printf("%e %f\n", tempResult, endT - startT);
    return 0;
}
